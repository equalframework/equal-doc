...
Each package can have a folder named ‘i18n’ containing, for each language, a subfolder using the IS0639 language code as naming convention (ex. : fr_BE or zh_CN). Inside those folders, for each class, translations are stored in `.json` files which prefixes are identical to the class they refer to (ex. : Student.json).

Those translation files use UTF-8 encoding and JSON format, and contain the translation terms of all items that might be translated (attributes ‘label’ and ‘help’).
