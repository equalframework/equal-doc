...
1)  translation of the values when depending on the language used for storing the Model;
2)  translation of the fields labels when objects are rendered within a View;
...
Note : as the only condition for the SQL type is to be compatible with the associated eQual type, in order for a binary field to be translated (for instance a PDF doc available in different languages), the size of the should remain smaller than the SQL MEDIUMBLOB type (16 MB with MySQL).
...
"view": {
  ...
  "list.default": {
    "name": "Partenaires",
    "description": "Liste de base des partenaires existants.",
    "layout": {}
  },
  "form.payer": {
    "name": "Client",
  ...
  "list.customer": {
    "name": "Clients",
    "description": "Liste de base des clients existants.",
    "layout": {}
  },
  "list.contact": {
    "name": "Contacts",
...
"view": {
  ...
  "list.default": {
    ...
    "description": "Liste de base des identités existantes.",
    "layout": {}
  }
}
```
